...
if __name__ == "__main__" :
    from ui_preferencesDialogForConfigSetup import Ui_Dialog
...
class DialogData(Qt.QObject):
    ...
    def __init__(self, full_string_list, 
        ...
        self._updateSelection(selected_string_list)
        
    #----------------------------------------------------------------------
    def _updateSelection(self, selected_string_list):
        """"""
    ...
    #----------------------------------------------------------------------
    def _hasNoDuplicateStrings(self):
        """"""
        
        str_list = self.full_string_list
    ...
    def _moveSelectedItemsUp(self, strings_to_be_moved_up):
        ...
        selected_indices = [i for (i,TF) in 
                            enumerate(selection_flags)
                            if TF]
        
        update_only_SelectedList = True        
        self.emit(Qt.SIGNAL('readyForListsViewUpdate'),
                  selected_indices, update_only_SelectedList)
...
class DialogView(Qt.QDialog, Ui_Dialog):
    ...
    def accept(self):
        ...
        self.data.output = {'selected_list':selected_list}
...
class DialogApp(Qt.QObject):
    ...
    def _initFileManager(self):
        ...
        self.connect(self.fileManager,
                     ...
                     self.data._updateSelection)
        
    #----------------------------------------------------------------------
    def _prepareForPrefFileSave(self):
        """"""
        
        visible_col_name_list = self.data.strings_in_SelectedList
    ...
    def _prepareForPrefFileLoad(self):
        """
        There is nothing to prepare here. The only reason
        why the signal chain is passing here is to let
        the file manager know the class of the sender of the
        signal is this preferences dialog.
        """
