...
def init_ltb(grp):
    ...
    for i,rec in enumerate(d[1:]):
        ...
        grp[dsname].attrs["unitsys"] = ",".join([rec[7], "phy", ""])
        grp[dsname].attrs["direction"] = (rec[6], "A")
        grp[dsname].attrs["_class_"] = "polynomial"
        grp[dsname].attrs["_magtype_"] = rec[0]
        grp[dsname].attrs["elements"] = names
        iuc += 1
