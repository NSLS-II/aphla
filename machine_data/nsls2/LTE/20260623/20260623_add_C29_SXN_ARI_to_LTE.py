# Used pixi env "yh-apv2-2026-03-pyele"
# (manifest at ~/.ap_pixi_manifests/yh-apv2-2026-03-pyele/)
# to run this script.
...
def interactively_adjust_elements(
    ...
    target_se = {}
    new_Ls = {
        "IDC29H1": 1.8,   # [m]; upstream EPU50 (SXN) active length
        "IDC29H2": 1.82,  # [m]; downstream EPU70 (ARI) active length
    }
    ...
    target_sb["IDC29H1"] = target_se["IDC29H1"] - new_Ls["IDC29H1"]
    # CU1X/Y correctors have L=0, so DL02 ends where IDC29H1 starts:
    target_se["DL02G1C29A"] = target_sb["IDC29H1"]
    target_se["PU2G1C29A"] = straight_center_spos + target_dse["PU2G1C29A"]
    target_se["PU3G1C29A"] = straight_center_spos + target_dse["PU3G1C29A"]
    target_se["IDC29H2"] = (
        straight_center_spos + target_dsc["IDC29H2"] + new_Ls["IDC29H2"] / 2
    )
    target_sb["IDC29H2"] = target_se["IDC29H2"] - new_Ls["IDC29H2"]
    # CU1X/Y correctors have L=0, so DL06 ends where IDC29H2 starts:
    target_se["DL06G1C29A"] = target_sb["IDC29H2"]
    target_se["PU4G1C29A"] = straight_center_spos + target_dse["PU4G1C29A"]
    target_se["GEG1C29A"] = straight_end_spos

    # key: element whose L will be adjusted
    # value: element whose "se" will be adjusted to the target "se" after L adjustment
    # Note: DL04G1C29A is a fixed filler between DL03 and DL05; DL05G1C29A is
    #       adjusted to hit PU3G1C29A target (PU3 immediately follows DL05).
    target_names = {
        "DL01G1C29A": "PU1G1C29A",
...
def gen_new_layout_LTE_file(straight_cell_num, n_existing_ubpms, n_existing_ukickmaps):
    ...
    # *) Adding C29 SXN EPU50 (upstream) and ARI EPU70 (downstream) kickmap elements
    # *) Adding ID BPMs PU1-PU4 for C29 SXN/ARI


    # When running the next line, you may see many error messages like:
    #   Kickmap element "OVU68G1C20D": File "..." does not exist.
    ...
    # --- Upstream: SXN EPU50 ---
    old_u_name = "IDC29H1"
    new_u_name = "EPU50G1C29U"
    new_u_length = 1.8  # [m]
    new_u_period = 50e-3  # [m]
    # --- Downstream: ARI EPU70 ---
    old_d_name = "IDC29H2"
    new_d_name = "EPU70G1C29D"
    new_d_length = 1.82  # [m]
    new_d_period = 70e-3  # [m]

    elem_name_changes = {
        old_u_name: dict(name=new_u_name, update_in_beamline_def=True),
        old_d_name: dict(name=new_d_name, update_in_beamline_def=True),
    }
    elem_type_changes = {
        new_u_name: "UKICKMAP",  # from drift
        new_d_name: "UKICKMAP",  # from drift
        "PU1G1C29A": "MONI",  # from drift
        "PU2G1C29A": "MONI",  # from drift
        "PU3G1C29A": "MONI",  # from drift
        "PU4G1C29A": "MONI",  # from drift
    }
    nkicks_u = int(np.ceil(new_u_length / new_u_period))
    if nkicks_u % 2 == 1:
        nkicks_u += 1  # Make it even, in case this kickmap needs to be split half later
    nkicks_d = int(np.ceil(new_d_length / new_d_period))
    if nkicks_d % 2 == 1:
        nkicks_d += 1  # Make it even, in case this kickmap needs to be split half later
    other_new_props = [
        dict(
            elem_name=new_u_name,
            prop_name="INPUT_FILE",
            prop_val=f'"{KM_OFFICIAL_LOCAL_FOLDER_STR}/SXN_EPU50LV_1o8m_T2m2_woKm2sdds.sdds"',
        ),
        ...
        dict(
            ...
            prop_name="INPUT_FILE",
            prop_val=f'"{KM_OFFICIAL_LOCAL_FOLDER_STR}/ARI_EPU70LV_1o82m_T2m2_woKm2sdds.sdds"',
        ),
    ...
    # *) Splitting kickmap elements at C03, C09, C16, and C17 into half and
    #    inserting X-BPMs at the middle
    # Note: C29 EPUs are NOT split (no half-length kickmap files available yet).
    n_new_ubpms = n_existing_ubpms + 4  # Added 4 BPMs for C29 SXN/ARI
...
def gen_insertion_device_states_yaml():
    ...
    for layout_type in ["new", "new_w_xbpms"]:
        ...
        # All 22 non-DW IDs, i.e., C27 HEX, C20 IFE, C09 CDI, C29 SXN & ARI all closed
        state_name = layout_type.replace("new", "22ids")
        states[state_name] = dict(elem_type_changes={}, elem_prop_changes=[])

        common_args = (states, km_elem_names, elem_names, elem_defs, LTE)

        # Out of all 22 non-DW IDs, only C27 HEX is opened.
        state_name = layout_type.replace("new", "22ids_wo_HEX")
        id_elem_names_to_open = ["SCW70G1C27D"]
        _add_new_ids_state(state_name, id_elem_names_to_open, *common_args)

        # Out of all 22 non-DW IDs, only C20 IFE is opened.
        state_name = layout_type.replace("new", "22ids_wo_IFE")
        id_elem_names_to_open = ["OVU68G1C20D"]
        _add_new_ids_state(state_name, id_elem_names_to_open, *common_args)

        # Out of all 22 non-DW IDs, only C20 IFE & C27 HEX are opened.
        state_name = layout_type.replace("new", "22ids_wo_IFE_HEX")
...
def validate_new_LTE_files(
    ...
    new_n_ukickmaps = {
        "bare": 0,
        "3dw": 2 * 3,  # 2 kickmap elements at C08, C18, C28
        "22ids": orig_n_ukickmaps + len(new_kickmap_elem_names),
    }
    new_n_ukickmaps["22ids_wo_HEX"] = new_n_ukickmaps["22ids"] - 1
    new_n_ukickmaps["22ids_wo_IFE"] = new_n_ukickmaps["22ids"] - 1
    new_n_ukickmaps["22ids_wo_IFE_HEX"] = new_n_ukickmaps["22ids"] - 2
    ...
    # C29 EPUs are NOT split (no half-length kickmap files available yet).
    for k in ['22ids', '22ids_wo_HEX', '22ids_wo_IFE', '22ids_wo_IFE_HEX']:
    ...
    for lat_name, d in twi_new_models.items():
        ...
        elif ids_state.startswith("22ids"):
    ...
    for k, ref_lat in LTE_files['ids_quads_state'].items():
        ...
        for new_lat in LTE_files['new_model']:
            ...
            # None of the reference lattices can agree with "22ids" new models
            # because they contain the C29 SXN/ARI kickmaps. So, exclude those here.
            if new_lat.ids_state.startswith("22ids"):
                continue
...
if __name__ == "__main__":
    ...
    # In this script, we're adding C29 SXN (EPU50, upstream) and ARI (EPU70, downstream).
    straight_cell_num = 29

    # As of 06/23/2026 with the layout including up to C09 CDI (20250905).
    # In this script, we will add 4 new ID BPMs for C29 SXN/ARI. So, next time you
    # update this script, this number should be raised to 53.
    n_existing_ubpms = 49

    # As of 06/23/2026 with the layout including up to C09 CDI (20250905).
    #
    # ID Cell # (excluding the one(s) being added in this script):
    #  I(O)VU: 3, 4, 5, 7-1, 9, 10, 11, 12, 16, 17-1, 17-2, 19, 20 (total 13)
    #  EPU: 2, 7-2, 21-1, 21-2, 23-1, 23-2 (total 6)
    #  SCW: 27 (total 1)
    #  DW: 8, 18, 28 (total 3)
    # Note:
    #   Non-DW 18th ID = HEX (C27)
    #   Non-DW 19th ID = IFE (C20)
    #   Non-DW 20th ID = CDI (C09)
    #   Non-DW 21st ID = SXN EPU50 (C29U, being added in this script)
    #   Non-DW 22nd ID = ARI EPU70 (C29D, being added in this script)
    #
    # There are 3 damping wigglers (2 each) plus 20 non-DW IDs before new addition.
    # In this script, we will add 2 new kickmaps for C29 SXN/ARI. So, next time you
    # update this script, `_n_ex_non_dw_ukickmaps` should be raised to 22.
    ...
    funcs_to_run = {
        ...
        "validate_new_LTE_files": False,
        "match_quads": False,
        "publicize_new_kickmap_files": False,
        "publicize_new_model_LTE_files": False,
    }

    # The following LTE file should be the latest layout file (without "_w_xbpms")
    # at the time of updating/running this script.
    # As of 06/23/2026, it is "../20260304/new_LTEs/20260304_aphla_layout_RelKMPaths.lte".
    # (The 20260304 script added new skew quads and produced a layout file whose
    # kickmap paths are already correct relative to this working directory.)
    # So, next time you update this script, it should be changed to:
    #  path=Path("../20260623/new_LTEs/20260623_aphla_layout_RelKMPaths.lte")
    LTE_files['layout']['orig'] = LayoutLatticeFile(
        path=Path("../20260304/new_LTEs/20260304_aphla_layout_RelKMPaths.lte"),
        with_xbpms=False,
    )
    ...
    for ids_state, quads_state in [
        ("22ids_wo_IFE_HEX", "17ids_matched_VS"),
        ("22ids_wo_HEX", "17ids_matched_VS"),
    ]:
        f = IDsQuadsStateLatticeFile(
    ...
    for ids_state, quads_state in [
        ("22ids_wo_IFE", "18ids_matched_YL"),
        ("22ids", "18ids_matched_YL"),
    ]:
        f = IDsQuadsStateLatticeFile(
    ...
    #
    # "ids_state=22ids" means 22 non-DW IDs (up to C29 SXN & ARI) are closed.
    ...
    if funcs_to_run["gen_insertion_device_states_yaml"]:
        # This function also needs to be updated every time when this script is updated.
    ...
    if funcs_to_run["validate_new_LTE_files"]:
        new_ubpm_names = ["PU1G1C29A", "PU2G1C29A", "PU3G1C29A", "PU4G1C29A"]
        new_kickmap_elem_names = ["EPU50G1C29U", "EPU70G1C29D"]
    ...
    if funcs_to_run["publicize_new_kickmap_files"]:
        new_official_kickmap_filenames = ['SXN_EPU50LV_1o8m_T2m2_woKm2sdds.sdds',
                                          'ARI_EPU70LV_1o82m_T2m2_woKm2sdds.sdds']
