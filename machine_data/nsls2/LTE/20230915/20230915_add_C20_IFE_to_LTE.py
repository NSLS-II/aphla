...
KM_OFFICIAL_LOCAL_FOLDER_RE_PATTERN = "\./kickmaps/official/"
...
def interactively_adjust_elements(
    ...
    # (i.e., se = 0 at the straight beginning)
    target_se = {"MK4G1C20A": straight_center_spos}
    new_Ls = {
        "IDC20H2": 3.4,  # [m]; from the length of the kickmap
    }
    ...
    # the straight center.
    target_dse = {"PU1G1C20A": -3.66234, "PU4G1C20A": +3.67766}
    # target_dsc [m]: s-pos of the element center w.r.t. the straight center
    # (i.e., dsc = 0 at the straight center)
    target_dsc = {"IDC20H2": +1.88416}

    target_se["PU1G1C20A"] = straight_center_spos + target_dse["PU1G1C20A"]
    target_se["IDC20H2"] = (
        straight_center_spos + target_dsc["IDC20H2"] + new_Ls["IDC20H2"] / 2
    )
    target_sb["IDC20H2"] = target_se["IDC20H2"] - new_Ls["IDC20H2"]
    target_se["PU3G1C20A"] = target_sb["IDC20H2"]
    target_se["PU4G1C20A"] = straight_center_spos + target_dse["PU4G1C20A"]
    target_se["GSG2C20A"] = straight_end_spos

    # key: element whose L will be adjusted
    # value: element whose "se" will be adjusted to the target "se" after L adjustment
    target_names = {
        "DH01G1C20A": "PU1G1C20A",
        "DH04G1C20A": "MK4G1C20A",
        "DH05G1C20A": "PU3G1C20A",
        "IDC20H2": "IDC20H2",
        "DH07G1C20A": "PU4G1C20A",
        "DH08G1C20A": "GSG2C20A",
    }
...
def gen_new_layout_LTE_file(straight_cell_num, n_existing_ubpms, n_existing_ukickmaps):
    ...
    for layout_type in ["new_layout", "new_layout_w_xbpms"]:
        ...
        (
            ...
            orig_L,
            max_name_len,
        ) = basic_lattice_integrity_check(base_LTE, straight_cell_num, n_existing_ubpms, n_existing_ukickmaps)

        # Interactively adjust this section to generate a desirable table
        #
        # *) Adding C20 IFE ID Kickmap element
        # *) Adding ID BPMs P7 & P8 for C20 IFE ID
        #
        # Additionaly only for "new_layout_w_xbpms":
        # *) Splitting kickmap elements at C03, C16, and C17 into half and inserting
        ...
        old_id_name = "IDC20H2"
        new_id_name = "OVU68G1C20D"
        elem_name_changes = {
            old_id_name: dict(name=new_id_name, update_in_beamline_def=True)
        }
        elem_type_changes = {
            new_id_name: "UKICKMAP",  # from drift
            "PU1G1C20A": "MONI",  # from drift
            "PU4G1C20A": "MONI",  # from drift
        }
        nkicks = int(np.ceil(3.4 / 68e-3))
        other_new_props = [
            dict(
                elem_name=new_id_name,
                prop_name="INPUT_FILE",
                prop_val=f'"{KM_OFFICIAL_LOCAL_FOLDER_STR}/U68kickmap_3o4m_T2m2_woKm2sdds.sdds"',
            ),
            ...
            dict(elem_name=new_id_name, prop_name="KREF", prop_val="4.5"),
        ]
...
def validate_new_LTE_files(load_from_proc, straight_cell_num, n_existing_ubpms, n_existing_ukickmaps,
    ...
    new_n_ukickmaps = {
        ...
        "19ids": orig_n_ukickmaps + len(new_kickmap_elem_names),
        "19ids_w_xbpms": orig_n_ukickmaps + len(new_kickmap_elem_names) + 4
        # ^ "+4" kickmaps from the kickmaps at C03, C16, C17-1, C17-2 split into half
    }
    for lat_name, d in twi.items():
        ...
        assert len(found_new_ukickmap_names) == new_n_ukickmaps[id_state_name]
        ...
        assert all(
            [
                (name in found_new_ukickmap_names) or (f"{name}_HALF" in found_new_ukickmap_names)
        ...
        assert np.all(
            ...
            == np.sort(new_kickmap_elem_names)
        )
    ...
    expected_Ls = {"OVU68G1C20D": 3.4}
    expected_ses = {
        "OVU68G1C20D": straight_center_spos + 1.88416 + expected_Ls["OVU68G1C20D"] / 2,
        "PU1G1C20A": straight_center_spos - 3.66234,
        "PU4G1C20A": straight_center_spos + 3.67766,
    }
    for lat_name in [
        "new_layout",
        "new_model:bare:bare",
    ...
    expected_ses["PX1G1C03A"] = 79.1958
...
def publicize_new_model_LTE_files(exist_ok=False):
    ...
    public_LTE_folderpath = APHLA_SR_PYELE_MODELS_FOLDER / "LTEs"
...
if __name__ == "__main__":
    ...
    LTE_fps["orig_layout"] = "20221027_aphla_layout_RelKMPaths.lte"
    ...
    # In this script, we're adding C20 IFE ID.
    straight_cell_num = 20

    # As of 09/14/2023 with the layout including up to C27 HEX.
    # In this script, we will add 2 new ID BPMs for C20 IFE. So, next time you update this
    # script, this number should be raised to 47.
    n_existing_ubpms = 45

    # As of 09/14/2023 with the layout including up to C27 HEX.
    # There are 3 damping wigglers (2 each) plus 18 non-DW IDs.
    # In this script, we will add one new kickmap for C20 IFE. So, next time you update this
    # script, this number should be raised to (3*2 + 19).
    n_existing_ukickmaps = 3 * 2 + 18

    new_ubpm_names = ["PU1G1C20A", "PU4G1C20A"]
    new_kickmap_elem_names = ["OVU68G1C20D"]

    # Run the following functions one by one in this order while adjusting the script
    if False:
        gen_magnet_props_yaml()
    if False:
        gen_new_layout_LTE_file(straight_cell_num, n_existing_ubpms, n_existing_ukickmaps)
    if False:
    ...
    if False:
        new_official_kickmap_filenames = ['U68kickmap_3o4m_T2m2_woKm2sdds.sdds']
