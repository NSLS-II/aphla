"""
Beta beat computation: 18-ID lattice (20 IDs minus C09 CDI and C20 IFE)
vs. 3DW base lattice.
...
# 20-ID lattice (contains C09 CDI as IVU18G1C09CM and C20 IFE as OVU68G1C20D)
LTE_20ids_filepath = SCRIPT_DIR / "new_LTEs" / (
    "20260304_aphla_20ids_Q_18ids_matched_YL_RelKMPaths.lte"
)
...
mod_LTE_filepath = SCRIPT_DIR / (
    "20260304_aphla_18ids_Q_18ids_matched_YL_RelKMPaths_wo_C09CDI_C20IFE.lte"
)

# Output pgz files (written to cwd = SCRIPT_DIR)
twi_18ids_pgz = SCRIPT_DIR / "twi_18ids_wo_C09CDI_C20IFE.pgz"
...
if not mod_LTE_filepath.exists():
    make_modified_lte(LTE_20ids_filepath, mod_LTE_filepath)
else:
...
if not twi_18ids_pgz.exists():
    run_twiss(mod_LTE_filepath, twi_18ids_pgz)
else:
    print(f"18-ID twiss already exists: {twi_18ids_pgz}")
...
twi_18ids = extract_bpm_twiss(twi_18ids_pgz)
twi_3dw   = extract_bpm_twiss(twi_3dw_pgz)

# Verify BPM names match between the two lattices
assert np.all(twi_18ids["names"] == twi_3dw["names"]), \
...
betax_beat = (twi_18ids["betax"] / twi_3dw["betax"] - 1.0) * 1e2  # [%]
betay_beat = (twi_18ids["betay"] / twi_3dw["betay"] - 1.0) * 1e2  # [%]

rms_betax = np.std(betax_beat)
rms_betay = np.std(betay_beat)

# BPMs immediately adjacent to the HEX ID (SCW70G1C27D in C27):
#   PL1G6C26B = last P[HLM]* BPM before HEX (upstream)
#   PL1G2C27A = first P[HLM]* BPM after HEX (downstream)
HEX_NEARBY_BPMS = {"PL1G6C26B", "PL1G2C27A"}
hex_excl_mask = np.array([n not in HEX_NEARBY_BPMS for n in twi_18ids["names"]])
rms_betax_xhex = np.std(betax_beat[hex_excl_mask])
rms_betay_xhex = np.std(betay_beat[hex_excl_mask])

fig, axes = plt.subplots(2, 1, figsize=(14, 8), sharex=True)

hex_excl_inds = bpm_index[~hex_excl_mask]

ax = axes[0]
ax.plot(bpm_index, betax_beat, "b.-", ms=4)
ax.plot(hex_excl_inds, betax_beat[~hex_excl_mask], "o", ms=8,
        mfc="none", mec="orange", mew=1.5, label="excl. from HEX-neighbor RMS")
ax.axhline(0.0, color="k", lw=0.8, ls="--")
ax.set_ylabel(
    r"$(\beta_x^{18\mathrm{ID}} / \beta_x^{3\mathrm{DW}} - 1) \times 100$ [%]"
)
ax.set_title(
    "Beta beat: 18-ID (wo C09 CDI & C20 IFE) vs. 3DW base\n"
    f"18ID: $\\nu_x={twi_18ids['nux']:.4f}$, $\\nu_y={twi_18ids['nuy']:.4f}$  |  "
    f"3DW: $\\nu_x={twi_3dw['nux']:.4f}$, $\\nu_y={twi_3dw['nuy']:.4f}$\n"
    f"rms $\\Delta\\beta_x/\\beta_x$ = {rms_betax:.2f}% (all), "
    f"{rms_betax_xhex:.2f}% (excl. HEX neighbors)  |  "
    f"rms $\\Delta\\beta_y/\\beta_y$ = {rms_betay:.2f}% (all), "
    f"{rms_betay_xhex:.2f}% (excl. HEX neighbors)"
)
ax.grid(True, lw=0.4)

ax = axes[1]
ax.plot(bpm_index, betay_beat, "r.-", ms=4)
ax.plot(hex_excl_inds, betay_beat[~hex_excl_mask], "o", ms=8,
        mfc="none", mec="orange", mew=1.5)
ax.axhline(0.0, color="k", lw=0.8, ls="--")
ax.set_ylabel(
    r"$(\beta_y^{18\mathrm{ID}} / \beta_y^{3\mathrm{DW}} - 1) \times 100$ [%]"
)
...
fig.tight_layout()
plot_path = SCRIPT_DIR / "20260324_beta_beat_18ids_wo_C09CDI_C20IFE.png"
