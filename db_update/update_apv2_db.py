...
BPM_PV_SUFFIX = {
    ...
    "put": {
        ...
        "xref1": "Pos:Href-SP",
        "yref1": "Pos:Vref-SP",
        "xref2": "Pos:UsrXoffset-SP",
        "yref2": "Pos:UsrYoffset-SP",
    },
}
...
def print_elems_around_straight(cell_num, n=5):
    ...
    all_elems_nearby_cells = ap.getElements(
        f"*c{(cell_num-1 if cell_num >= 2 else 30):02d}*"
...
def update_C27_straight(exist_ok=False):
    ...
    for info in bpm_info_list + id_info_list:
        ...
        if "UBPM" in info["groups"]:
            ...
            mainI_pvrb1 = (
                "SR:C26-MG{PS:HEX-Main}I:Ps1DCCT1-I"  # main current actual RB (DCCT1)
            )
            mainI_pvrb2 = (
                "SR:C26-MG{PS:HEX-Main}I:Ps1DCCT2-I"  # main current actual RB (DCCT2)
            )
            ...
            mainI_speed_pvsp = (
                "SR:C26-MG{PS:HEX-Main}Rate:Rmp1-SP"  # main current ramp rate SP
            )
            #
            ...
            output_cor_pvsps = [
                f"SR:C26-MG{{PS:HEX-IDMPS}}U{i}-I-SP" for i in range(1, 6 + 1)
            ] + [
            ...
            quad_pvrbs = [
                "SR:C26-MG{PS:QL1B}I:Ps1DCCT1-I",
    ...
    for plane in ["x", "y"]:
        for e in sel_scors:
            if (not exist_ok) and (f"{plane}_2nd" in d[e.name]["map"]):
                print(
                    f'Field "{plane}_2nd" already defined for "{e.name}". Not changing.'
                )
                continue
